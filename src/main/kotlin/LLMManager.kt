...
class LLMManager {
    ...
    suspend fun onCommand(
        ...
        val userMessage =
            if (messageContents.size >= 2) {
                messageContents.toMutableList().drop(1).joinToString(" ")
            } else {
                ""
            }
        ...
        val botResponse =
            try {
                generation(userMessage, message.author!!.username, message)
            } catch (e: IOException) {
                e.toString()
            }
    ...
    suspend fun onPing(message: Message): String {
        ...
        return try {
            generation(message.content.removePrefix("<@${kord?.selfId}>").trim(), message.author!!.username, message)
        } catch (
            e: IOException,
        ) {
            e.toString()
